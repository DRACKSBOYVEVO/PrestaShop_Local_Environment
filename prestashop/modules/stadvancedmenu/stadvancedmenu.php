...
require (dirname(__FILE__).'/StAdvancedMenuClass.php');
require (dirname(__FILE__).'/StAdvancedColumnClass.php');
require (dirname(__FILE__).'/StAdvancedProductClass.php');
require (dirname(__FILE__).'/StAdvancedBrandClass.php');

class StAdvancedMenu extends Module
{
    protected static $cache_stadvancedmenu;
	...
	public function __construct()
	{
		...
		$this->displayName   = $this->l('Advanced megamenu');
    ...
    public function sampleData($id_shop)
    {
		...
		$samples = array(
            ...
            1 => array(
                ...
                'html' => '', 
                'columns' => array(
                    0 => array('id_st_advanced_column' => 0, 'width' => 4, ),
                    1 => array('id_st_advanced_column' => 0, 'width' => 4, ),
                    2 => array('id_st_advanced_column' => 0, 'width' => 4, ),
                ),
            ),
            2 => array(
                'sample_pid' => 1,
                ...
                'item_t' => 5, 
                'title' => '', 
                'html' => '<h6 class="uppercase color_444" style="font-family:\'Fjalla One\';">Welcome to transformer theme</h6><p>Transformer theme is an elegant, powerful and fully responsive prestashop theme with modern design. Suitable for every type of store.</p><ul class="desc"><li>Fully Customizable Design</li><li>Powerful theme editor</li><li>Unlimited colors</li></ul><p><a class="go" title="Buy this theme" href="#" target="_blank">BUY THIS THEME</a></p>', 
            ),
            3 => array(
                'sample_pid' => 1,
                'sample_cid' => 1,
            ...
            4 => array(
                ...
                'item_t' => 5, 
                'title' => '', 
                'html' => '<p><a href="#" title="Transformer theme" rel="nofollow"><img src="/modules/stadvancedmenu/views/img/sample_2.jpg" alt="Transformer theme"/></a><p><p>Transformer theme is an elegant, powerful and fully responsive prestashop theme with modern design. Suitable for every type of store. Transformer theme is fully responsive, it looks stunning on all types of screens and devices.</p>', 
            ),
		);		
		foreach($samples as $k=>&$sample)
		{
			$module = new StAdvancedMenuClass();
            if($sample['id_st_advanced_column']===0)
                $id_st_advanced_column = 0;
            else
                $id_st_advanced_column = $samples[$sample['sample_pid']]['columns'][$sample['sample_cid']]['id_st_advanced_column'];

            $module->id_st_advanced_column = (int)$id_st_advanced_column;
			...
			$module->active = 1;
			$module->position = $k*20;
			$module->id_shop = (int)$id_shop;
			$return &= $module->add();
            if($return)
            {
                $sample['id_st_advanced_menu'] = $module->id;
                if(isset($sample['columns']) && count($sample['columns']))
                    foreach ($sample['columns'] as $ck => $column) {
                        $col = new StAdvancedColumnClass();
                        $col->id_st_advanced_menu = $module->id;
                        $col->width = $column['width'];
                        $col->active = 1;
                        $col->position = $ck;
                        $return &= $col->add();
                        if($return)
                            $sample['columns'][$ck]['id_st_advanced_column'] = $col->id;
                    }
            }
		}
		return $return;
	...
	public function installDb()
	{
		...
		$return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_advanced_menu` (
                `id_st_advanced_menu` int(10) UNSIGNED NOT NULL AUTO_INCREMENT,
                `location` tinyint(1) unsigned NOT NULL DEFAULT 0,
				`id_st_advanced_column` int(10) UNSIGNED NOT NULL DEFAULT 0,
				...
				PRIMARY KEY (`id_st_advanced_menu`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;');
		
        $return &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_advanced_menu_lang` (
                `id_st_advanced_menu` INT UNSIGNED NOT NULL AUTO_INCREMENT,
                `id_lang` int(10) unsigned NOT NULL ,
                `title` varchar(255) DEFAULT NULL,
                `link` varchar(255) DEFAULT NULL,
                `html` text,
                `cate_label` varchar(255) DEFAULT NULL,
                PRIMARY KEY (`id_st_advanced_menu`, `id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;');

        $return &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_advanced_column` (
                `id_st_advanced_column` INT UNSIGNED NOT NULL AUTO_INCREMENT,
                `id_st_advanced_menu` int(10) unsigned NOT NULL,
                `width` float(3,1) unsigned NOT NULL DEFAULT 4,
                `position` int(10) unsigned NOT NULL DEFAULT 0,
                `active` tinyint(1) unsigned NOT NULL DEFAULT 1,
                `hide_on_mobile` tinyint(1) unsigned NOT NULL DEFAULT 0, 
                `title` varchar(255) DEFAULT NULL,
                PRIMARY KEY (`id_st_advanced_column`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;');

        $return &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_advanced_menu_product` (
                `id_st_advanced_menu` int(10) unsigned NOT NULL,
                `id_product` int(10) unsigned NOT NULL,
                KEY `menu_product` (`id_st_advanced_menu`,`id_product`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;');

        $return &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'st_advanced_menu_brand` (
                `id_st_advanced_menu` int(10) unsigned NOT NULL,
                `id_manufacturer` int(10) unsigned NOT NULL,
                KEY `menu_brand` (`id_st_advanced_menu`,`id_manufacturer`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8 ;');
	...
	{
        return Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'st_advanced_menu`,`'._DB_PREFIX_.'st_advanced_menu_lang`,`'._DB_PREFIX_.'st_advanced_column`,`'._DB_PREFIX_.'st_advanced_menu_brand`,`'._DB_PREFIX_.'st_advanced_menu_product`');
	}
	...
	public function getContent()
	{
    	...
    	$id_st_advanced_menu = (int)Tools::getValue('id_st_advanced_menu');
		...
		if (isset($_POST['savestadvancedmenu']) || isset($_POST['savestadvancedmenuAndStay']))
        {
            if($id_st_advanced_menu)
				$menu = new StAdvancedMenuClass($id_st_advanced_menu);
			else
				$menu = new StAdvancedMenuClass();
            ...
            if (!count($error) && $menu->validateFields(false) && $menu->validateFieldsLang(false))
            {
                ...
                {
                    $this->clearStAdvancedmenuCache();
                    if(isset($_POST['savestadvancedmenuAndStay']) || Tools::getValue('fr') == 'view')
                    {
                        $rd_str = isset($_POST['savestadvancedmenuAndStay']) && Tools::getValue('fr') == 'view' ? 'fr=view&update' : (isset($_POST['savestadvancedmenuAndStay']) ? 'update' : 'view');
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_menu='.$menu->id.'&conf='.($id_st_advanced_menu?4:3).'&'.$rd_str.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules')); 
                    }   
                    else
                        $this->_html .= $this->displayConfirmation($this->l('Main menu').' '.($id_st_advanced_menu ? $this->l('updated') : $this->l('added')));
                }
                else
                    $this->_html .= $this->displayError($this->l('An error occurred during main menu').' '.($id_st_advanced_menu ? $this->l('updating') : $this->l('creation')));
            }
			else
				$this->_html .= count($error) ? implode('',$error) : $this->displayError($this->l('Invalid value for field(s).'));
        }

        if (isset($_POST['savecolumnstadvancedmenu']) || isset($_POST['savecolumnstadvancedmenuAndStay']))
		{
            $id_st_advanced_column = (int)Tools::getValue('id_st_advanced_column');
            if($id_st_advanced_column)
                $column = new StAdvancedColumnClass($id_st_advanced_column);
            else
                $column = new StAdvancedColumnClass();
            ...
            if (!count($error) && $column->validateFields(false) && $column->validateFieldsLang(false))
            {
                ...
                {
                    $this->clearStAdvancedmenuCache();
                    if(isset($_POST['savecolumnstadvancedmenuAndStay']))
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_column='.$column->id.'&conf='.($id_st_advanced_column?4:3).'&updatestadvancedcolumn&token='.Tools::getAdminTokenLite('AdminModules'));    
                    else
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_menu='.$column->id_st_advanced_menu.'&view'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));
                }
                else
                    $this->_html .= $this->displayError($this->l('An error occurred during menu').' '.($id_st_advanced_column ? $this->l('updating') : $this->l('creation')));
            }
            else
                $this->_html .= count($error) ? implode('',$error) : $this->displayError($this->l('Invalid value for field(s).'));

        }
        if (isset($_POST['savecustomlinkstadvancedmenu']) || isset($_POST['savecustomlinkstadvancedmenuAndStay']))
        {
            if($id_st_advanced_menu)
				$menu = new StAdvancedMenuClass($id_st_advanced_menu);
			else
            ...
            $menu_parent = new StAdvancedMenuClass($menu->id_parent);
            $menu->level_depth = $menu_parent->level_depth+1;
            
            $menu->item_k = $menu->item_v = '';
            $item = Tools::getValue('links');
            if($item)
            {
                $item_arr = explode('_',$item);
            ...
            if (!count($error) && $menu->validateFields(false) && $menu->validateFieldsLang(false))
            {
                ...
                {
                    $this->clearStAdvancedmenuCache();
                    if(isset($_POST['savecustomlinkstadvancedmenuAndStay']))
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_menu='.$menu->id.'&conf='.($id_st_advanced_menu?4:3).'&update'.$this->name.'&id_parent='.$menu->id_st_advanced_column.'&ct=4&token='.Tools::getAdminTokenLite('AdminModules'));    
                    else
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_column='.$menu->id_st_advanced_column.'&viewstadvancedcolumn&token='.Tools::getAdminTokenLite('AdminModules'));
                }
                else
                    $this->_html .= $this->displayError($this->l('An error occurred during menu').' '.($id_st_advanced_menu ? $this->l('updating') : $this->l('creation')));
            }
			else
				$this->_html .= count($error) ? implode('',$error) : $this->displayError($this->l('Invalid value for field(s).'));
        }
		if (isset($_POST['savecustomcontentstadvancedmenu']) || isset($_POST['savecustomcontentstadvancedmenuAndStay']))
        {
            if($id_st_advanced_menu)
				$menu = new StAdvancedMenuClass($id_st_advanced_menu);
			else
            ...
            if (!count($error) && $menu->validateFields(false) && $menu->validateFieldsLang(false))
            {
                ...
                {
                    $this->clearStAdvancedmenuCache();
                    if(isset($_POST['savecustomcontentstadvancedmenuAndStay']))
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_menu='.$menu->id.'&conf='.($id_st_advanced_menu?4:3).'&update'.$this->name.'&id_parent='.$menu->id_st_advanced_column.'&token='.Tools::getAdminTokenLite('AdminModules'));    
                    else
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_column='.$menu->id_st_advanced_column.'&viewstadvancedcolumn&token='.Tools::getAdminTokenLite('AdminModules'));
                }
                else
                    $this->_html .= $this->displayError($this->l('An error occurred during custom content').' '.($id_st_advanced_menu ? $this->l('updating') : $this->l('creation')));
            }
			else
				$this->_html .= count($error) ? implode('',$error) : $this->displayError($this->l('Invalid value for field(s).'));
        }
        if (isset($_POST['savecategorystadvancedmenu']) || isset($_POST['savecategorystadvancedmenuAndStay']))
        {
            if($id_st_advanced_menu)
                $menu = new StAdvancedMenuClass($id_st_advanced_menu);
            else
            ...
            if (!count($error) && $menu->validateFields(false) && $menu->validateFieldsLang(false))
            {
                ...
                {
                    $this->clearStAdvancedmenuCache();
                    if(isset($_POST['savecategorystadvancedmenuAndStay']))
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_menu='.$menu->id.'&conf='.($id_st_advanced_menu?4:3).'&update'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));    
                    else
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_column='.$menu->id_st_advanced_column.'&viewstadvancedcolumn&token='.Tools::getAdminTokenLite('AdminModules'));
                }
                else
                    $this->_html .= $this->displayError($this->l('An error occurred during menu item').' '.($id_st_advanced_menu ? $this->l('updating') : $this->l('creation')));
            }
            else
                $this->_html .= count($error) ? implode('',$error) : $this->displayError($this->l('Invalid value for field(s).'));
        }
        if (isset($_POST['saveproductstadvancedmenu']) || isset($_POST['saveproductstadvancedmenuAndStay']))
        {
            if($id_st_advanced_menu)
                $menu = new StAdvancedMenuClass($id_st_advanced_menu);
            else
            ...
            if (!count($error) && $menu->validateFields(false) && $menu->validateFieldsLang(false))
            {
                if($menu->save())
                {
                    StAdvancedProductClass::deleteMenuProducts($menu->id);
                    $products_id = array_unique(explode('-', $products));
                    if (count($products_id))
                        StAdvancedProductClass::changeMenuProducts($menu->id, $products_id);

                    $this->clearStAdvancedmenuCache();
                    if(isset($_POST['saveproductstadvancedmenuAndStay']))
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_menu='.$menu->id.'&conf='.($id_st_advanced_menu?4:3).'&update'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));    
                    else
                        Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_column='.$menu->id_st_advanced_column.'&viewstadvancedcolumn&token='.Tools::getAdminTokenLite('AdminModules'));
                }
        ...
        if (isset($_POST['savebrandstadvancedmenu']) || isset($_POST['savebrandstadvancedmenuAndStay']))
        {
            if($id_st_advanced_menu)
                $menu = new StAdvancedMenuClass($id_st_advanced_menu);
            else
        ...
        if(Tools::getValue('act')=='delete_image' && $identi = Tools::getValue('id_st_advanced_menu'))
        {
            $result = array(
                'r' => false,
                'm' => '',
            ...
            $menu = new StAdvancedMenuClass((int)$identi);
	    ...
	    if ((Tools::isSubmit('activestadvancedmenu')))
        {
    		$menu = new StAdvancedMenuClass((int)$id_st_advanced_menu);
        ...
        if ((Tools::isSubmit('activestadvancedcolumn')))
        {
            $id_st_advanced_column = (int)Tools::getValue('id_st_advanced_column');
    		$column = new StAdvancedColumnClass($id_st_advanced_column);
        ...
        elseif (Tools::isSubmit('addmenustadvancedmenu'))
		{
        ...
        elseif (Tools::isSubmit('updatestadvancedmenu'))
        {
    		$menu = new StAdvancedMenuClass((int)$id_st_advanced_menu);
        ...
        elseif (Tools::isSubmit('updatestadvancedcolumn'))
        {
            $id_st_advanced_column = (int)Tools::getValue('id_st_advanced_column');
            $column = new StAdvancedColumnClass((int)$id_st_advanced_column);
        ...
        else if (Tools::isSubmit('deletestadvancedmenu'))
		{
    		$menu = new StAdvancedMenuClass((int)$id_st_advanced_menu);
            if(Validate::isLoadedObject($menu))
            {
                if($menu->id_parent)
                    $menu_secondary_id = StAdvancedMenuClass::getSecondaryParent((int)$menu->id);
        ...
        else if (Tools::isSubmit('deletestadvancedcolumn'))
		{
            $id_st_advanced_column = (int)Tools::getValue('id_st_advanced_column');
    		$column = new StAdvancedColumnClass($id_st_advanced_column);
            if(Validate::isLoadedObject($column))
        ...
        elseif(Tools::isSubmit('viewstadvancedmenu'))
        {
            $menu = new StAdvancedMenuClass((int)$id_st_advanced_menu);
            if(!Validate::isLoadedObject($menu))
                Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));
            ...
            $all = StAdvancedColumnClass::getAll($menu->id, 0);
            $this->_html .= $helper->generateList($all, $this->fields_list);
            
            return $this->_html;
        }
        elseif(Tools::isSubmit('viewstadvancedcolumn'))
        {
            $id_st_advanced_column = (int)Tools::getValue('id_st_advanced_column');
    		$column = new StAdvancedColumnClass((int)$id_st_advanced_column);
            if(!Validate::isLoadedObject($column))
            ...
            $jon = StAdvancedMenuClass::getByColumnId($column->id, $this->context->language->id, 0, 0, 0);
            $all = array();
            foreach ($jon  as $k) {
                $all[] = $k;
                if($k['item_t']==4)
                {
                    $li = StAdvancedMenuClass::recurseTree($k['id_st_advanced_menu'],0,0,0,$this->context->language->id, 4);
                    ...
                    $cs = StAdvancedMenuClass::recurseTree($k['id_st_advanced_menu'],1,0,0,$this->context->language->id, 5);
        ...
        elseif (Tools::isSubmit('addcolumnstadvancedmenu')) {
            $id_parent = (int)Tools::getValue('id_parent');
            if(!$id_parent)
                Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));
            $menu = new StAdvancedMenuClass($id_parent);
            if(!Validate::isLoadedObject($menu))
                Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));
        ...
        else
        {
            ...
            $all = StAdvancedMenuClass::recurseTree(0,1,0,0,$this->context->language->id,0);
    ...
    public function getCustomLinkContent(&$li)
    {
        ...
        if(is_array($li) && count($li)) 
            foreach($li as &$v)
            {
                ...
                $cc = StAdvancedMenuClass::recurseTree($v['id_st_advanced_menu'],1,0,0,$this->context->language->id, 5);
    ...
    private function _toFlat(&$res, $arr, $cid=0)
    {
        if(is_array($arr) && count($arr)) 
            foreach($arr as $v)
            {
                if($cid && $v['id_st_advanced_menu']==$cid)
                    continue;
                $tmp=$v;
                unset($tmp['children']);
                $res[] = $tmp;
    ...
    public function createLinks($icon=true)
    {
        ...
        $links = array(
            ...
            'product' => array('name'=>$this->l('Products'),'query'=>array(
	...
	private function initForm()
    {
                ...
                array(
					...
					'values' => array(
						...
						array(
							'id' => 'hide_on_mobile_1',
							'value' => 1,
							'label' => $this->l('Visible on large devices (screen width > 992px)')),
                        array(
							'id' => 'hide_on_mobile_2',
							'value' => 2,
							'label' => $this->l('Visible on small devices (screen width < 992px)')),
					),
                ),
                array(
                    'type' => 'text',
        ...
        if(Validate::isLoadedObject($menu))
        {
            ...
            if ($menu->bg_image)
                $this->fields_form[1]['form']['input']['bg_image_field']['image'] = '<img src="'._THEME_PROD_PIC_DIR_.$this->name.'/thumb'.$menu->bg_image.'" class="img_preview">
                    <p><a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_menu='.$menu->id.'&token='.Tools::getAdminTokenLite('AdminModules').'" class="btn btn-default st_delete_image"><i class="icon-trash"></i> Delete</a></p>';
        }
		...
		$helper->submit_action = 'savestadvancedmenu';
    ...
    public function getParentList($id_st_advanced_column, $cid)
    {
        $result = array();
        $parents = StAdvancedMenuClass::getByColumnId($id_st_advanced_column, $this->context->language->id, 0, 4, 0);
        if(is_array($parents) && count($parents))
        {
            foreach($parents as &$v)
            {
                $jon = StAdvancedMenuClass::recurseTree($v['id_st_advanced_menu'],0,0,$active=0,$this->context->language->id, 4);
    ...
    public function recurseParents($v, $parents)
    {
        ...
        {
            if($v['id_st_advanced_menu'] == $value['id_parent'])
            {
                $value = $this->recurseParents($value, $parents);
                $v['children'][$value['id_st_advanced_menu']] = $value;
            }
        }
        return $v;
    }
    public function initCategoryForm()
    {
        ...
        {
            $menu = new StAdvancedMenuClass((int)$id_st_advanced_menu);
            $id_parent = $menu->id_st_advanced_column;
        }
        else
            $menu = new StAdvancedMenuClass();
        ...
        $helper->identifier = $this->identifier;
        $helper->submit_action = 'savecategorystadvancedmenu';
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
            'fields_value' => $this->getFieldsValueSt($menu),
        ...
        $helper->tpl_vars['fields_value']['id_st_advanced_column'] = $id_parent;
    ...
    public function initProductForm()
    {
        ...
        $menuProducts = StAdvancedProductClass::getMenuProductsLight($this->context->language->id, $menu->id);
        ...
        $helper->identifier = $this->identifier;
        $helper->submit_action = 'saveproductstadvancedmenu';
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
            'fields_value' => $this->getFieldsValueSt($menu),
    ...
    public function initBrandForm()
    {
        ...
        $helper->identifier = $this->identifier;
        $helper->submit_action = 'savebrandstadvancedmenu';
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
            'fields_value' => $this->getFieldsValueSt($menu),
        ...
        $manufacturers_html = '';
        if ($res = StAdvancedBrandClass::getByMenu((int)$menu->id))
            foreach($res AS $value)
            {
    ...
    public function initCustomLinkForm()
    {
        ...
        {
            $this->fields_form[0]['form']['input'][] = array('type' => 'hidden', 'name' => 'id_st_advanced_menu');
            if($menu->item_k == 2 && $menu->item_v)
                $this->fields_form[0]['form']['input']['links']['options']['optiongroup']['query']['product']['query'][0] = array('id' => '2_'.$menu->item_v, 'name' => 'ID product '.$menu->item_v);
        }
        $helper = new HelperForm();
        $helper->show_toolbar = false;
        $helper->module = $this;
        ...
        $helper->identifier = $this->identifier;
        $helper->submit_action = 'savecustomlinkstadvancedmenu';
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
            'fields_value' => $this->getFieldsValueSt($menu),
    ...
    public function initCustomContentForm()
    {
        ...
        $helper->identifier = $this->identifier;
        $helper->submit_action = 'savecustomcontentstadvancedmenu';
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
            'fields_value' => $this->getFieldsValueSt($menu),
    ...
    protected function initList()
	{
		$this->fields_list = array(
			'title' => array(
				...
				'callback_object' => 'StAdvancedMenu',
                'search' => false,
                'orderby' => false,
			),
			'item_k' => array(
    ...
    public function initColumnList()
    {
        ...
        $id_st_advanced_menu = (int)Tools::getValue('id_st_advanced_menu');
        $menu = new StAdvancedMenuClass((int)$id_st_advanced_menu, $this->context->language->id);
        if(!$menu->is_mega)
        ...
        $helper->shopLinkType = '';
        $helper->simple_header = false;
        $helper->identifier = 'id_st_advanced_column';
        $helper->actions = array('view', 'edit', 'delete');
        $helper->show_toolbar = true;
        $helper->toolbar_btn['new'] =  array(
            'href' => AdminController::$currentIndex.'&configure='.$this->name.'&addcolumn'.$this->name.'&id_parent='.(int)Tools::getValue('id_st_advanced_menu').'&token='.Tools::getAdminTokenLite('AdminModules'),
    ...
    private function initColumnForm()
    {
        ...
        {
            $column = new StAdvancedColumnClass((int)$id_st_advanced_column);
            $id_parent = $column->id_st_advanced_menu;
        }
        else
            $column = new StAdvancedColumnClass();
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'hidden',
                    'name' => 'id_st_advanced_menu',
                ),
                array(
                    'type' => 'html',
                    'id' => 'a_cancel',
                    'label' => '',
                    'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_menu='.$id_parent.'&view'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> Back to list</a><a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> Back to home page</a>',    
                ),
                array(
                    'type' => 'hidden',
                    'name' => 'fr',
        ...
        if(Validate::isLoadedObject($column))
        {
            $this->fields_form[0]['form']['input'][] = array('type' => 'hidden', 'name' => 'id_st_advanced_column');
        }

        $parent = new StAdvancedMenuClass($id_parent);
        if(!$parent->is_mega)
        ...
        $helper->identifier = $this->identifier;
        $helper->submit_action = 'savecolumnstadvancedmenu';
        $helper->currentIndex = $this->context->link->getAdminLink('AdminModules', false).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name;
        $helper->token = Tools::getAdminTokenLite('AdminModules');
        $helper->tpl_vars = array(
        ...
        $helper->tpl_vars['fields_value']['id_st_advanced_menu'] = $id_parent;
    ...
    {
        if($value)
            return StAdvancedMenu::displayTitle($value, $row);
        else
        {
            $module = new StAdvancedMenu();
    ...
    public static function displayItemTitle($value, $row)
    {
        ...
        switch ($row['item_t']) {
            ...
            case 4:
                ...
                break;
            case 2:
                $menuProducts = StAdvancedProductClass::getMenuProductsLight($context->language->id, $row['id_st_advanced_menu']);
                if(is_array($menuProducts) && count($menuProducts))
                {
            ...
            case 3:
                ...
                {
                    $menuBrands = StAdvancedBrandClass::getMenuBrandsLight($context->language->id, $row['id_st_advanced_menu']);
    ...
    {
        $id_parent = (int)Tools::getValue('id_st_advanced_column');
        $column = new StAdvancedColumnClass($id_parent);
        if(!Validate::isLoadedObject($column))
            Tools::redirectAdmin(AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'));
        ...
        $leval0 = new StAdvancedMenuClass($column->id_st_advanced_menu, $this->context->language->id);
		...
		$helper->toolbar_btn['back'] =  array(
			'href' => AdminController::$currentIndex.'&configure='.$this->name.'&id_st_advanced_menu='.$column->id_st_advanced_menu.'&view'.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules'),
			'desc' => $this->l('Back')
		);
        $helper->tpl_vars['navigate'] = array(
            '<a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'">'.$this->l("Home").'</a>',
            '<a href="'.AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'&id_st_advanced_menu='.$leval0->id.'&view'.$this->name.'">'.self::displayTitle($leval0->title, get_object_vars($leval0)).'</a>',
    ...
    {
		$this->context->controller->addJS($this->_path.'views/js/stadvancedmenu.js');
		$this->context->controller->addJqueryPlugin('hoverIntent');
        if (!$this->isCached('header.tpl', $this->getCacheId()))
        {
            $custom_css = '';
            $data = StAdvancedMenuClass::getCustomCss();
            if(is_array($data) && count($data))
                foreach($data as $v)
                {
                    ...
                    if($v['bg_color'])
                        $custom_css .= '#st_advanced_menu_wrap #st_advanced_ma_'.$v['id_st_advanced_menu'].',#st_advanced_menu_column_block #st_advanced_ma_'.$v['id_st_advanced_menu'].'{background-color:'.$v['bg_color'].';}';
                    if($v['bg_color_over'])
                        $custom_css .= '#st_advanced_menu_wrap #st_advanced_ma_'.$v['id_st_advanced_menu'].':hover, #st_advanced_menu_wrap #st_advanced_menu_'.$v['id_st_advanced_menu'].'.current .advanced_ma_level_0,#st_advanced_menu_column_block #st_advanced_ma_'.$v['id_st_advanced_menu'].':hover, #st_advanced_menu_column_block #st_advanced_menu_'.$v['id_st_advanced_menu'].'.current .advanced_ma_level_0{background-color:'.$v['bg_color_over'].';}';
                    if($v['tab_content_bg'])
                        $custom_css .= '#st_advanced_menu_wrap #st_advanced_menu_'.$v['id_st_advanced_menu'].' .stadvancedmenu_sub, #st_advanced_menu_wrap #st_advanced_menu_'.$v['id_st_advanced_menu'].' .stadvancedmenu_multi_level ul, #st_advanced_menu_wrap #st_advanced_menu_'.$v['id_st_advanced_menu'].' .advanced_mu_level_2 ul,#st_advanced_menu_column_block #st_advanced_menu_'.$v['id_st_advanced_menu'].' .stadvancedmenu_sub, #st_advanced_menu_column_block #st_advanced_menu_'.$v['id_st_advanced_menu'].' .stadvancedmenu_multi_level ul, #st_advanced_menu_column_block #st_advanced_menu_'.$v['id_st_advanced_menu'].' .advanced_mu_level_2 ul,#st_advanced_menu_wrap #st_advanced_menu_'.$v['id_st_advanced_menu'].' .stadvancedmenu_vs{background-color:'.$v['tab_content_bg'].';}';
                    if($v['bg_image'])
                    {
                        ...
                        $custom_css .= '#st_advanced_menu_'.$v['id_st_advanced_menu'].' .stadvancedmenu_sub,#st_advanced_menu_'.$v['id_st_advanced_menu'].' .stadvancedmenu_vs{background-image:url('.$bg_img.');';
                        switch($v['bg_repeat']) {
                            case 1 :
                    ...
                    if($v['bg_margin_bottom'])
                        $custom_css .= '#st_advanced_menu_'.$v['id_st_advanced_menu'].' .stadvancedmenu_sub,#st_advanced_menu_'.$v['id_st_advanced_menu'].' .stadvancedmenu_vs{padding-bottom:'.($v['bg_margin_bottom']+20).'px;}';

                    if($v['cate_label_color'])
                        $custom_css .= '#st_advanced_ma_'.$v['id_st_advanced_menu'].' .cate_label,#st_mo_advanced_ma_'.$v['id_st_advanced_menu'].' .cate_label{color:'.$v['cate_label_color'].';}';
                    if($v['cate_label_bg'])
                        $custom_css .= '#st_advanced_ma_'.$v['id_st_advanced_menu'].' .cate_label,#st_mo_advanced_ma_'.$v['id_st_advanced_menu'].' .cate_label{background-color:'.$v['cate_label_bg'].';}';   
                }
            $this->smarty->assign('advancedmenu_custom_css', $custom_css);
        }
    ...
    {
        $all = StAdvancedMenuClass::recurseTree(0,1,0,1,$this->context->language->id,0);
        
        if(is_array($all) && count($all))
        {
            foreach($all as &$v)
            {
                $columns = StAdvancedColumnClass::getAll($v['id_st_advanced_menu'], 1);
                if($v['is_mega'])
                ...
                {
                    $jon = StAdvancedMenuClass::getByColumnId($col['id_st_advanced_column'], $this->context->language->id, 1, 0, 0);
                    ...
                    foreach ($jon  as $k)
                    {
                        switch ($k['item_t']) {
                            case '1':
                                ...
                                break;
                            case '2':
                                $menuProducts = StAdvancedProductClass::getMenuProducts($this->context->language->id, $k['id_st_advanced_menu']);
                                $listProducts = array();
                                if ($menuProducts)
                                {
                                    ...
                                    foreach ($menuProducts as $product)
                                    {
                                        ...
                                        if(isset($curProduct->specificPrice['reduction']) && $curProduct->specificPrice['reduction'])
                                            $curProduct->price_without_reduction = $curProduct->getPriceWithoutReduct($tax_calc==1 ? true : false);
                            ...
                            case '3':
                                if ($k['item_k'] == 1)
                                    ...
                                    $menuBrands = StAdvancedBrandClass::getMenuBrands($this->context->language->id, $k['id_st_advanced_menu']);
                                $k['children'] = $menuBrands;
                                break;
                            case '4':
                                $cs_children = array();
                                $li = StAdvancedMenuClass::recurseTree($k['id_st_advanced_menu'],0,0,1,$this->context->language->id, 4);
                                if(is_array($li) && count($li))
                                {
                                    $this->getCustomLinkContent($li);
                                    $cs_children = array_merge($cs_children, $li);
                                }
                                $li = StAdvancedMenuClass::recurseTree($k['id_st_advanced_menu'],1,0,1,$this->context->language->id, 5);
                                if(is_array($li) && count($li))
                                {
                                     $cs_children = array_merge($cs_children, $li);
                                }
    ...
    {
        $cache_id = 'stadvancedmenu::setLastVisitedCategory';
	...
	{
	    $rs = Db::getInstance()->getValue('SELECT COUNT(0) FROM '._DB_PREFIX_.'st_advanced_menu_product 
            WHERE id_st_advanced_menu = (SELECT id_st_advanced_menu FROM '._DB_PREFIX_.'st_advanced_menu_product 
            WHERE id_product='.(int)$params['object']->id.') AND id_product != '.(int)$params['object']->id);
        if (!$rs)
        {
            $rs = Db::getInstance()->executeS('SELECT id_st_advanced_menu 
            FROM '._DB_PREFIX_.'st_advanced_menu_product
            WHERE id_product = '.(int)$params['object']->id);
            foreach($rs AS $value)
            {
                $menu = new StAdvancedMenuClass($value['id_st_advanced_menu']);
                $menu->delete();
            }
        }
    ...
    private function clearStAdvancedmenuCache()
    {
		$this->_clearCache('*');
    }
