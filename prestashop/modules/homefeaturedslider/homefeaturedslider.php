...
class HomeFeaturedSlider extends Module
{
	...
	function install()
	{
		if (!Configuration::updateValue('HOME_FEATURED_S_NBR_MOD', 8) 
            ...
            || !Configuration::updateValue('HOME_FEATURED_S_DISPLAY_PRO_COL', 0) 
            ...
            || !Configuration::updateValue('HOME_FEATURED_S_DISPLAY_SD', 0)
            || !Configuration::updateValue('HOME_FEATURED_S_GRID', 0)
            || !Configuration::updateValue('STSN_FEATURED_PRO_PER_LG_0', 4)
            || !Configuration::updateValue('STSN_FEATURED_PRO_PER_MD_0', 4)
            || !Configuration::updateValue('STSN_FEATURED_PRO_PER_SM_0', 3)
            || !Configuration::updateValue('STSN_FEATURED_PRO_PER_XS_0', 2)
            || !Configuration::updateValue('STSN_FEATURED_PRO_PER_XXS_0', 1)
            //
	...
	{            
        ...
        $this->fields_form[1]['form']['input']['featured_pro_per_0']['name'] = $this->BuildDropListGroup($this->findCateProPer());
    ...
    public function initFieldsForm()
    {
        ...
        $this->fields_form[1]['form'] = array(
			...
			'input' => array(
                ...
                'featured_pro_per_0' => array(
                    'type' => 'html',
                    'id' => 'featured_pro_per_0',
    ...
    public function hookDisplayHeader($params)
    {
        if (!$this->isCached('header.tpl', $this->getCacheId()))
        {
            ...
            if($top_margin || $top_margin!==null)
                $custom_css .= '.featured_products_sldier_block_center_container{margin-top:'.(int)$top_margin.'px;}';
            ...
            if ($title_font_size = (int)Configuration::get($this->_prefix_st.'TITLE_FONT_SIZE'))
            {
                 ...
                 $custom_css .= '.featured_products_sldier_block_center_container .nav_top_right .flex-direction-nav{top:-'.(round((round($title_font_size*1.3)-24)/2)+24+22).'px;}';
            }
            ...
            if ($direction_color = Configuration::get($this->_prefix_st.'DIRECTION_COLOR'))
                $custom_css .= '.featured_products_sldier_block_center_container .flex-direction-nav a{color:'.$direction_color.';}';
            if ($direction_bg = Configuration::get($this->_prefix_st.'DIRECTION_BG'))
                $custom_css .= '.featured_products_sldier_block_center_container .flex-direction-nav a{background-color:'.$direction_bg.';}';
            if ($direction_hover_bg = Configuration::get($this->_prefix_st.'DIRECTION_HOVER_BG'))
                $custom_css .= '.featured_products_sldier_block_center_container .flex-direction-nav a:hover{background-color:'.$direction_hover_bg.';}';
            if ($direction_disabled_bg = Configuration::get($this->_prefix_st.'DIRECTION_DISABLED_BG'))
                $custom_css .= '.featured_products_sldier_block_center_container .flex-direction-nav a.flex-disabled{background-color:'.$direction_disabled_bg.';}';
    ...
    public function hookDisplayHome($params, $hook_hash = '', $flag=0)
    {
        ...
        if (Configuration::get('HOME_FEATURED_S_GRID') || !$this->isCached('homefeaturedslider.tpl', $this->stGetCacheId($hook_hash)))
        {
            ...
            $this->smarty->assign(array(
                ...
                'hook_hash'              => $hook_hash,
                'pro_per_lg'       => (int)Configuration::get('STSN_FEATURED_PRO_PER_LG_0'),
                'pro_per_md'       => (int)Configuration::get('STSN_FEATURED_PRO_PER_MD_0'),
                'pro_per_sm'       => (int)Configuration::get('STSN_FEATURED_PRO_PER_SM_0'),
                'pro_per_xs'       => (int)Configuration::get('STSN_FEATURED_PRO_PER_XS_0'),
                'pro_per_xxs'       => (int)Configuration::get('STSN_FEATURED_PRO_PER_XXS_0'),
        ...
        return $this->display(__FILE__, 'homefeaturedslider.tpl', Configuration::get('HOME_FEATURED_S_GRID') ? NULL : $this->stGetCacheId($hook_hash));
    }
	...
	public function hookDisplayLeftColumn($params, $hook_hash = '')
	{
	    if (!$hook_hash)
            $hook_hash = $this->getHookHash(__FUNCTION__);
	    if (!$this->isCached('homefeaturedslider.tpl', $this->stGetCacheId($hook_hash)))
    	{
		...
		return $this->display(__FILE__, 'homefeaturedslider.tpl', $this->stGetCacheId($hook_hash));
	}
    ...
    private function _prepareHook($col=0)
    {
        ...
        {
            $featured_category_id = (int)Configuration::get('HOME_FEATURED_S_CAT');
		...
		*/
        ...
        $items = Configuration::get('HOME_FEATURED_S_ITEMS_COL');
        ...
        $display_sd = Configuration::get('HOME_FEATURED_S_DISPLAY_SD');
        ...
        $this->smarty->assign(array(
            ...
            'display_pro_col'       => Configuration::get('HOME_FEATURED_S_DISPLAY_PRO_COL'),
    ...
    public function hookDisplayFooter($params, $hook_hash = '')
    {
        if (!$hook_hash)
            $hook_hash = $this->getHookHash(__FUNCTION__);
	    if (!$this->isCached('homefeaturedslider-footer.tpl', $this->stGetCacheId($hook_hash)))
	    {
		...
		return $this->display(__FILE__, 'homefeaturedslider-footer.tpl', $this->stGetCacheId($hook_hash));
    }
    ...
    private function getConfigFieldsValues()
    {
        $fields_values = array(
            ...
            'slideshow' => Configuration::get('HOME_FEATURED_S_SLIDESHOW'),
            's_speed' => Configuration::get('HOME_FEATURED_S_S_SPEED'),
            'a_speed' => Configuration::get('HOME_FEATURED_S_A_SPEED'),
            'pause' => Configuration::get('HOME_FEATURED_S_PAUSE'),
            'loop' => Configuration::get('HOME_FEATURED_S_LOOP'),
            'move' => Configuration::get('HOME_FEATURED_S_MOVE'),
            'soby' => Configuration::get('HOME_FEATURED_S_SOBY'),
            'hide_mob' => Configuration::get('HOME_FEATURED_S_HIDE_MOB'),
            'display_sd' => Configuration::get('HOME_FEATURED_S_DISPLAY_SD'),
            'grid'=> Configuration::get('HOME_FEATURED_S_GRID'),
            
            'display_pro_col' => Configuration::get('HOME_FEATURED_S_DISPLAY_PRO_COL'),
            'nbr_mod_col' => Configuration::get('HOME_FEATURED_S_NBR_MOD_COL'),
            'easing_col' => Configuration::get('HOME_FEATURED_S_EASING_COL'),
            'slideshow_col' => Configuration::get('HOME_FEATURED_S_SLIDESHOW_COL'),
            's_speed_col' => Configuration::get('HOME_FEATURED_S_S_SPEED_COL'),
            'a_speed_col' => Configuration::get('HOME_FEATURED_S_A_SPEED_COL'),
            'pause_col' => Configuration::get('HOME_FEATURED_S_PAUSE_COL'),
            'loop_col' => Configuration::get('HOME_FEATURED_S_LOOP_COL'),
            'move_col' => Configuration::get('HOME_FEATURED_S_MOVE_COL'),
            'items_col' => Configuration::get('HOME_FEATURED_S_ITEMS_COL'),
            'soby_col' => Configuration::get('HOME_FEATURED_S_SOBY_COL'),
            'hide_mob_col' => Configuration::get('HOME_FEATURED_S_HIDE_MOB_COL'),
            
            'nbr_fot' =>Configuration::get('HOME_FEATURED_S_NBR_FOT'),
            'soby_fot' =>Configuration::get('HOME_FEATURED_S_SOBY_FOT'),    
