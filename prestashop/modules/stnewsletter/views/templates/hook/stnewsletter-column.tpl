...
{if isset($news_letter_array) && $news_letter_array|@count > 0}
    {foreach $news_letter_array as $ec}
		<section id="st_news_letter_{$ec.id_st_news_letter}" class="st_news_letter_{$ec.id_st_news_letter} {if $ec.hide_on_mobile} hidden-xs{/if} block">
    		<h3 class="title_block"><span>{l s='Newsletter' mod='stnewsletter'}</span></h3>
			<div class="{if $ec.text_align==2} text-center {elseif $ec.text_align==3} text-right {/if}">
